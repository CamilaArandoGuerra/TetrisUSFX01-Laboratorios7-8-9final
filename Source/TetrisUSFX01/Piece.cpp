...
APiece::APiece()
{
    ...
    TArray<AActor*> LPieza; UGameplayStatics::GetAllActorsOfClass(GetWorld(), APiece::StaticClass(), LPieza);
    if (LPieza.Num() > 5)
    {
        //If exist at least one of them, set the instance with the first found one
        limitar = Cast<APiece>(LPieza[5]);
        GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Yellow, FString::Printf(TEXT("%s ya existen 5 piezas"), *limitar->GetName()));
...
void APiece::SpawnBlocks()
{
    ...
    // Search for existing Instances of this class
        ...
        ABlock* B = nullptr;
        FString _TipoBloque = ABlock::getBloque();

        if (_TipoBloque == "BlockPiedra")
        {
            for (auto&& YZ : YZs)
            {
